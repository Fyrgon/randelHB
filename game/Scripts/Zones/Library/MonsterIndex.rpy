...
label monsterindex:
    ...
    if bom == "":
        mc "Ugh... I forgot..."
        jump library
    if bom.strip().lower().replace(' ', '') in ("boar", "boars"):
    ...
    if bom.strip().lower().replace(' ', '') in ("falcon", "falcons"):
    ...
    if bom.strip().lower().replace(' ', '') in ("alphafalcon", "alphafalcons"):
    ...
    if bom.strip().lower().replace(' ', '') in ("goblin", "goblins"):
    ...
    if bom.strip().lower().replace(' ', '') in ("wallcrawler", "wallcrawlers"):
    ...
    if bom.strip().lower().replace(' ', '') in ("imp", "imps"):
    ...
    if bom.strip().lower().replace(' ', '') in ("vampire", "vampires"):
    ...
    if bom.strip().lower().replace(' ', '') in ("lelluk", "lelluks"):
        mc "There is nothing on the Lelluk."
        if sawcynth >= 5:
            mc "I guess Cynthia wasn't lying about it being super rare."
        jump library

    if bom.strip().lower().replace(' ', '') in ("direwolf", "direwolfs", "direwolves"):
    ...
    if bom.strip().lower().replace(' ', '') in ("ledric", "ledrics"):
    ...
    if bom.strip().lower().replace(' ', '') in ("troll", "trolls"):
    ...
    if bom.strip().lower().replace(' ', '') in ("bloodbutterfly", "bloodbutterflys", "bloodbutterflies"):
    ...
    if bom.strip().lower().replace(' ', '') in ("glob", "globs"):
        "SOMEONE IS TOO LAZY TO WRITE THIS RIGHT NOW."
        jump library
        $ time += 1
        play sound chime
